...
for m=1:length(SNR_BPSK)
    ...
    %Tx
    tx_BPSK=randi([0 1],1,nsymbol_BPSK);
    Dn_BPSK=C_BPSK(tx_BPSK+1);
    
    %Channel
    hn=1/sqrt(2)*(randn(1,nsymbol_BPSK)+1i*randn(1,nsymbol_BPSK));
    sigma_BPSK=sqrt(0.5/SNR_lin_BPSK(m));
    wn=sigma_BPSK*(randn(1,nsymbol_BPSK)+1i*randn(1,nsymbol_BPSK));
    rn=hn.*Dn_BPSK+wn;
    %Rx
    for n=1:nsymbol_BPSK
        dn_r_BPSK(n)=real(rn(n)./hn(n));
        if dn_r_BPSK(n)<=0
            bn_r_BPSK(n)=1;
        else
            bn_r_BPSK(n)=0;
    ...
    %BER
    new_errors=biterr(bn_r_BPSK,tx_BPSK);
    total_errors_BPSK=total_errors_BPSK+new_errors;
    BER_sim_BPSK(m)=total_errors_BPSK/nsymbol_BPSK;
    toc
end
...
sigma_16QAM=sqrt(0.5.*spow./symbol_bits./SNR_lin_16QAM);
...
for m=1:length(SNR_16QAM)
    ...
    %---ifft---
    tx_signal_time=ifft(Dn_16qam_mod)*sqrt(nsymbol_16QAM);
    %---Insert Cyclic Prefix---
    dn_cp=[tx_signal_time(:,1:cp_length),tx_signal_time];
    
    %=======Channel========
    awgn_noise_025=0.25*(randn(1,nsymbol_16QAM)+1i*randn(1,nsymbol_16QAM));
    awgn_noise=sigma_16QAM(m)*(randn(1,nsymbol_16QAM+cp_length)+1i*randn(1,nsymbol_16QAM+cp_length));
    signal025=Dn_16qam_mod+awgn_noise_025;
    ...
    %---fft---
    rx_signal_freq=fft(rx_cp)/sqrt(nsymbol_16QAM);
    %---16QAM Demodulation---
    for l =1:nsymbol_16QAM
        C_Dk = sqrt(abs(C_16QAM-rx_signal_freq(l)).^2);
        [a,index] = min(C_Dk);
        rx(l) = index-1;
    end
    
    %======Compute BER======
    new_errors=biterr(rx,tx_16QAM);
    total_errors_16QAM=total_errors_16QAM+new_errors;
    BER_sim_16QAM(m)=total_errors_16QAM/nsymbol_16QAM/symbol_bits;
    toc
end
